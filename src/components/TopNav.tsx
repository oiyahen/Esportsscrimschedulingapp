...
export function TopNav({ onCreateScrim, profile }: TopNavProps) {
  // Pull data from profile with safe fallbacks
  const displayUsername = profile?.username ?? 'JohnDoe';
  const displayTeam = profile?.primary_team ?? 'Vanguard Prime';

  // Same initials logic as Profile
  const initials = (() => {
  ...
  // Short team tag for the little square (e.g. "Vanguard Prime" -> "VP")
  const teamTag = (() => {
